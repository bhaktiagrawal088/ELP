...
const getAccessToken = async () => {
    ...
    try {
        ...
        const tokenResponse = await axios_1.default.post(`${process.env.PHONEPE_BASE_URL}/v1/oauth/token`, new URLSearchParams({
            client_id: process.env.PHONEPE_CLIENT_ID,
...
const initiatePayment = async (req, res) => {
    try {
        ...
        const phonepeResponse = await axios_1.default.post(`${process.env.PHONEPE_BASE_URL}/checkout/v2/pay`, payload, {
            headers: {
                "Content-Type": "application/json",
                "Accept": "application/json",
                "Authorization": `O-Bearer ${token}`,
...
const paymentCallback = async (req, res) => {
    try {
        ...
        if (!base64Payload)
            return res.status(400).send("Missing response payload");
        const decodedPayload = JSON.parse(Buffer.from(base64Payload, "base64").toString("utf-8"));
        console.log("Decoded Callback Payload:", decodedPayload);
        const { merchantOrderId, state } = decodedPayload;
        if (state === "COMPLETED") {
            ...
            console.log("Course added to user:", userId);
            // Create order
            await orderModel_1.default.create({
                userId,
                courseId,
                payment_info: decodedPayload,
            });
